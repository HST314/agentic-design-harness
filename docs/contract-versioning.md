...
- 默认业务契约版本：`1.0`；TaskCard 与 TokenUsageEvent 消费者额外支持 `1.1`；
...
TokenUsageEvent 1.1 保留 1.0 Token 字段，并增加 Provider、调用类型、Provider request id、原始 usage 与非 Token 计费单位。图片调用以张数、分辨率和模型档位记录，不把缺失 Token 估算成文本 Token；1.0 事件继续按原 Schema 读取。
